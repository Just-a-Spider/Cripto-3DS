...
void prompt_for_amount(char* amount_buffer, int buffer_size) {
    SwkbdState swkbd;
    swkbdInit(&swkbd, SWKBD_TYPE_NUMPAD, 1, -1);
    swkbdSetValidation(&swkbd, SWKBD_NOTEMPTY_NOTBLANK, 0, 0);
    swkbdSetHintText(&swkbd, "Override Trade Amount (USDT)");
    swkbdSetInitialText(&swkbd, amount_buffer);
    
    SwkbdButton button = swkbdInputText(&swkbd, amount_buffer, buffer_size);
    if (button != SWKBD_BUTTON_CONFIRM) {
        amount_buffer[0] = '\0';
    }
}
